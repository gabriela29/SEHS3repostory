...
Namespace Bll
    ...
    Public Class subcategoriaManager
        Public Shared Function GetList(ByVal vCategoriaId As Integer, ByVal vNombre As String) As DataTable
            Return subcategoriaBD.GetList(vCategoriaId, vNombre)
        End Function

        Public Shared Function GetItem(ByVal SubCategoriaId As Integer) As subcategoria
            Return subcategoriaBD.GetItem(SubCategoriaId)
        End Function

        Public Shared Function Grabar(ByRef objsc As subcategoria) As Long
            Dim rs As DataTable, vCodigo As Long
            vCodigo = 0
            Try
                rs = subcategoriaBD.Grabar(objsc)
        ...
        Public Shared Function Eliminar(ByVal vCategoriaId As Integer) As Boolean
            Dim rs As DataTable
            Try
                rs = subcategoriaBD.Eliminar(vCategoriaId)
                If Not rs Is Nothing Then
                    If rs.DataSet.Tables(0).Rows.Count > 0 Then
                        If rs.DataSet.Tables(0).Rows(0).Item("outstate") Then
                            Eliminar = True
