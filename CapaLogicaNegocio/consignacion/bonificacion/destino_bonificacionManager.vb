...
Namespace Bll
    Public Class destino_bonificacionManager

        Public Shared Function GetItem(ByVal codigo As Integer) As bonificacion_destino
            Return destino_bonificacionBD.GetItem(codigo)
        End Function

        Public Shared Function GetList(ByVal Descripcion As String) As DataTable
            Return destino_bonificacionBD.GetList(Descripcion)
        End Function

        Public Shared Function Grabar(ByRef objDB As bonificacion_destino) As Long
            Dim rs As DataTable, vCodigo As Long
            vCodigo = 0
            Try
                rs = destino_bonificacionBD.Grabar(objDB)
        ...
        Public Shared Function Eliminar(ByVal vCodigo As Integer) As Boolean
            Dim rs As DataTable
            Try
                rs = destino_bonificacionBD.Eliminar(vCodigo)
                If Not rs Is Nothing Then
                    If rs.DataSet.Tables(0).Rows.Count > 0 Then
                        If rs.DataSet.Tables(0).Rows(0).Item("outstate") Then
                            Eliminar = True
