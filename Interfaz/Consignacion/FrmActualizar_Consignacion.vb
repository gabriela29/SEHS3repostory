...
Public Class FrmActualizar_Consignacion
  Public pCodigo As Long, pCodigo_Per As Long, pCodigo_Alm As Integer, pNombre_Almacen As String, pTipo As Integer
    ...
    If e.KeyChar = Chr(Keys.Enter) Then
      ...
      If pTipo = 1 Then
        If lblBuscarpor.Tag = 1 Then 'nombre
          dtProducto = productoManager.Consultar_Registros_cosig(Now.Year, pCodigo_Alm, 10, txtBuscar.Text.Trim, "", 0, "", True)
        ElseIf lblBuscarpor.Tag = 2 Then 'codigo
          dtProducto = productoManager.Consultar_Registros_cosig(Now.Year, pCodigo_Alm, 10, "", "", txtBuscar.Text, "", True)
        ElseIf lblBuscarpor.Tag = 3 Then 'barras
          dtProducto = productoManager.Consultar_Registros_cosig(Now.Year, pCodigo_Alm, 10, "", "", 0, txtBuscar.Text.Trim, True)
        End If
      Else
        If lblBuscarpor.Tag = 1 Then 'nombre
          dtProducto = productoManager.Consultar_Registros(Now.Year, pCodigo_Alm, 10, txtBuscar.Text.Trim, "", 0, "", True)
        ElseIf lblBuscarpor.Tag = 2 Then 'codigo
          dtProducto = productoManager.Consultar_Registros(Now.Year, pCodigo_Alm, 10, "", "", txtBuscar.Text, "", True)
        ElseIf lblBuscarpor.Tag = 3 Then 'barras
          dtProducto = productoManager.Consultar_Registros(Now.Year, pCodigo_Alm, 10, "", "", 0, txtBuscar.Text.Trim, True)
        End If
      End If
  ...
  Public Function Actualizar() As Boolean
    ...
    Try
      ...
      With Objeto
        ...
        .Codigo_Campania = pCampaniaId
        .Codigo_Asistente = pCodigo_Per
        .fecha = vFecha
      ...
      If xok = consignacionManager.Agregar(pCodigo, Objeto, vArray) Then
        If cboDocumento.Value = 4 Then
          Dim frm As New FrmVisor_Consignacion
          frm.Imprimir_Guia(Long.Parse(xok), pCodigo_Alm, cboDocumento.Value)
  ...
  Private Sub txtRucDni_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtRucDni.KeyDown
    If e.KeyCode = Keys.Enter Then
      ...
      ObjPe = personaManager.Datos_Persona_Colportaje("N", pCodigo_Alm, pCodigo_Per, txtRucDni.Text)
